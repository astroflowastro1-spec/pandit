    ...
    const fetchProducts = async () => {
      try {
        ...
        if (data.success && data.data.length > 0) {
          ...
          if (currentCategory) {
            filtered = data.data.filter((p: any) => p.category === currentCategory);
          }
          // fallback to all products if category filter yields nothing
      ...
      <div className="container mx-auto px-4 md:px-8 max-w-7xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {products.map((item, idx) => {
            ...
            const price = isReady 
              ? (isIndia ? item.priceInr : convertPrice(item.priceUsd)) 
              : item.priceInr;
                ...
                <div className="relative aspect-[4/5] bg-white rounded-[24px] overflow-hidden mb-4 shadow-sm border border-[#EAE3D5]">
                  ...
                  <div className="absolute inset-0 bg-black/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 gap-2">
                    <div className="translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 delay-75">
                      <Link href={`/product/${item.slug}`} className="w-full bg-white/95 backdrop-blur-sm hover:bg-orange-600 hover:text-white text-gray-900 font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2 shadow-sm text-sm">
                        <FiEye /> View Product
                      </Link>
                    </div>
                  </div>
